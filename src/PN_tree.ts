...
function convertBase(from:number, to:number, input:string, putZero:boolean) {
...
function transformBoard(board:string|number[][], compress:boolean, gridSize:number, getPossible:boolean) {
    if(compress) {
        var outputString = "";
        var boardString = "";
        var putZero = false;
        for(let i = 0; i < board[0].length; ++i) {
            ...
            if(outputString != "" || i == gridSize - 2) {putZero = true;}
        }
        return outputString;
    }
    //decompress
...
export class node {
    ...
    public type: string;
    constructor(parent:node, board:string, children:node[], proof:number, disproof:number) {
        this.parent = parent;
